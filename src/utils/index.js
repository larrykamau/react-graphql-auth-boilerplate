...
export const hashPassword = async secret => {
  ...
  addToLocalStorageObject("kiu_auth_profile", "hash", secretKey);
  addToLocalStorageObject("kiu_auth_profile", "secret", makeSecretKey(8));
  addToLocalStorageObject("kiu_auth_profile", "key", ciphertext);
};
export const unhashPassword = secret => {
  if (localStorage.getItem("kiu_auth_profile") === null) {
    ...
    JSON.parse(localStorage.getItem("kiu_auth_profile")).key !== undefined
  ) {
    const profileData = JSON.parse(localStorage.getItem("kiu_auth_profile"));
    ...
    var profile = JSON.parse(localStorage.kiu_auth_profile || {}); //read and convert to object
    ...
    localStorage.kiu_auth_profile = JSON.stringify(profile); //save it back
    return originalText;
  } else {
    return noText;
  }
